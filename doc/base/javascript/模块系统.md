...
- 同步加载：也被称为阻塞模式加载，这会使得宿主环境暂停对于其他资源的处理，直到当前资源文件加载并完毕。例如浏览器在JS文件加载并执行完毕前，会停止页面的渲染。
...
在浏览器中通常使用`<script>`标签加载JavaScript脚本。
...
- 在默认情况下，浏览器会按照文档顺序同步加载JavaScript脚本。这会阻塞浏览器的渲染进程，直到脚本加载并执行完毕，在继续进行渲染。这是因为浏览器并不确定加载的脚本会不会对DOM树产生操作，如果会，就会重新渲染，造成资源浪费。但是当脚本体积很大，下载和执行的时间变长，就会造成页面的卡顿、或者首屏白屏，导致不友好的体验。

- 在下面的代码中，给`<script>`添加了defer属性
